var camera = {
  ...
  update: function () {
    let point = Math.atan2(yMousePos - canvas.height / 2, xMousePos - canvas.width / 2);
    let distance =
      2 *
      Math.sqrt(
        Math.pow(xMousePos - canvas.width / 2, 2) + Math.pow(yMousePos - canvas.height / 2, 2)
      );
  ...
  check: function () {
    this.offSetX = (Math.round((this.offSetXNew - this.offSetXOld) * 10) / 50) * screenratio;
    this.offSetXOld = this.offSetXNew;
    this.offSetY = (Math.round((this.offSetYNew - this.offSetYOld) * 10) / 50) * screenratio;
...
var player = {
  ...
  inicialize: function (starterX, starterY) {
    ...
    this.companionsIndex = [0, 360 / this.companions, (2 * 360) / this.companions];
    this.shieldCD = [0, 0];
    this.attackCD = false;
    this.hitCD = false;
  ...
  update: function () {
    ...
    this.targetxspeed = (this.ratio * this.lastRatioX * this.accelerationX) / 100;
    this.targetyspeed = (this.ratio * this.lastRatioY * this.accelerationY) / 100;
    if (this.HP[0] <= 0 || this.playerLives == 0) {
      ...
      if (
        ...
        this.xspeed += ((this.targetxspeed >= 0 ? 1 : -1) * (this.accelerationX / 100)) / 2;
      } else {
        if (this.xspeed >= 0.5) {
          this.xspeed -= 0.5;
      ...
      if (
        ...
        this.yspeed += ((this.targetyspeed >= 0 ? 1 : -1) * (this.accelerationY / 100)) / 2;
      } else {
        if (this.yspeed >= 0.5) {
          this.yspeed -= 0.5;
    ...
    for (let i = 0; i < this.companions; i++) {
      this.companionsX[i] =
        this.x +
        companionDistance * screenratio * Math.cos((this.companionsIndex[i] / 180) * Math.PI);
      this.companionsY[i] =
        this.y +
        companionDistance * screenratio * Math.sin((this.companionsIndex[i] / 180) * Math.PI);
    }
  },
  render: function () {
    this.animationIndex += 1;
    let nextIndex = Math.round(
      300 / this.animationFPS / ((60 * (this.accelerationX + this.accelerationY)) / 200)
    );
    ...
    this.angle = Math.atan2(yMousePos - this.y, xMousePos - this.x) + Math.PI / 2;
    ...
    for (let i = 0; i < this.companions; i++) {
      ...
      else ctx.drawImage(sprite.player_companion, 0, 30, 30, 30, -30, -30, 60, 60);
      ctx.restore();
    }
  },
  ...
  hitCDstart: async function (playerEntity, event) {
    ...
    else if (playerEntity == 1 && event == "collision") this.collisionCD = false;
  },
  ...
  ratioByMouse: function () {
    ...
    if (!rightMouseDown) {
      this.ratio =
        this.currentSpeed /
        (Math.abs(xMousePos - canvas.width / 2) + Math.abs(yMousePos - canvas.height / 2));
  ...
  ratioByKeyboard: function (vectorx, vectory, event = false) {
    if (!event) {
      ...
      if (keyboardControler.buttonsActive == 1) {
        this.ratio =
          this.currentSpeed /
          (Math.abs(vectorx - canvas.width / 2) + Math.abs(vectory - canvas.height / 2));
